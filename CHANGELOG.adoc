...
=== Added
- Julia backend v0.8.0-dev (Step 1 complete)
  * BetLang.jl module with core bet primitives
  * Racket → Julia compiler
  * Test suite with 121 tests (all passing)
  * List operations (map, filter, fold)
  * Composition functions (chain, compose)
  * Statistical utilities (probability, entropy, expect, variance)
  * Integration with Distributions.jl, StatsBase.jl
  * Examples and documentation
- Design documents for Julia backend and ecosystem integration
- Updated all documentation to reflect v0.6 completion

=== Changed
- Version: 0.6.0 → 0.8.0-dev
- Overall completion: 80% → 82%
- Julia backend: 0% → 20% (Step 1 complete)

== [0.6.0] - 2026-02-07

=== Added
- **6 advanced number systems** completing all 14 uncertainty-aware types:
  * DistnumberBeta - Beta distributions for bounded probabilities
  * Hyperreal - Non-standard analysis with infinitesimals
  * SurrealAdvanced - Full surreal number arithmetic
  * PAdicAdvanced - Complete p-adic number system
  * ImpreciseProbability - Interval-valued probability bounds
  * DempsterShafer - Belief functions and evidence theory
- `RUST-STATUS.md` documenting Rust compiler blockers and solutions
- Updated `SAFETY-FEATURES.md` with all 14 number systems

=== Changed
- Overall completion: 65% → 80%
- Number systems: 45% → 100% (all 14 implemented)
- Racket implementation: 90% → 100% (production ready)
- Updated `README.adoc` with comprehensive status table
- Updated `STATE.scm` to reflect v0.6.0 completion
- Updated `ECOSYSTEM.scm` with betlang's ecosystem position

=== Fixed
- Rust compiler serde serialization issues (Arc<T> helpers)
- Rust compiler type system issues (lifetimes, PartialEq)
- Rust runtime API compatibility (im crate HashSet methods)

=== Status
- ✅ Racket implementation: **Production Ready**
- ✅ Safety features: Complete (4/4)
- ✅ Number systems: Complete (14/14)
- ⚠️ Rust compiler: Blocked by LALRPOP parser conflicts

== [0.5.0] - 2026-02-01

=== Added
- **4 safety features** for gambling harm reduction:
  * Dutch book prevention (`lib/dutch-book.rkt`)
  * Risk-of-ruin protection (`lib/risk-of-ruin.rkt`)
  * Cool-off mechanism (`lib/cool-off.rkt`)
  * 8 uncertainty-aware number systems (`lib/number-systems.rkt`)
- `SAFETY-FEATURES.md` comprehensive documentation
- `examples/safety-features.rkt` demonstration
- Kelly criterion optimal stake calculation
- Monte Carlo risk-of-ruin simulation
- Time-locked betting with violation tracking

=== Changed
- Overall completion: 40% → 65%
